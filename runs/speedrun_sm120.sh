...
torchrun --standalone --nproc_per_node=4 -m scripts.base_train -- --depth=24 --target-param-data-ratio=8 --device-batch-size=16 --fp8 --attn-impl flex --run=$WANDB_RUN
...
torchrun --standalone --nproc_per_node=4 -m scripts.chat_sft -- --attn-impl flex --run=$WANDB_RUN
